...
class AdminModel {
    ...
    private function migrar(): void {
        $cols = [
            ...
            "ALTER TABLE comercios ADD COLUMN IF NOT EXISTS idioma VARCHAR(5) NOT NULL DEFAULT 'es'",
        ];
        foreach ($cols as $sql) {
    ...
    // Token de un solo uso para que chefcontrol (otro subdominio) arranque la
    // sesión del admin del comercio, ya que las sesiones PHP no se comparten
    // entre chefcontrol-sup y chefcontrol.
    public function crearTokenImpersonacion(int $comercioId, int $usuarioId): array {
        try {
            $this->db->exec(
            ...
            // La expiración se calcula con NOW() de MySQL (no con PHP date()) para
            // evitar desfases si el reloj/zona horaria de PHP y el de MySQL difieren.
            $token = bin2hex(random_bytes(24));
            $this->db->prepare(
                "INSERT INTO impersonacion_tokens (token, usuario_id, comercio_id, expira_en)
                 VALUES (?, ?, ?, NOW() + INTERVAL 60 SECOND)"
            )->execute([$token, $usuarioId, $comercioId]);
    ...
    public function actualizarIdioma(int $id, string $idioma): array {
        $idiomasValidos = ['es', 'en', 'pt'];
        if (!in_array($idioma, $idiomasValidos)) {
            return ['ok' => false, 'msg' => 'Idioma no válido.'];
        }
        try {
            $this->db->prepare(
                "UPDATE comercios SET idioma=? WHERE id=?"
            )->execute([$idioma, $id]);
            return ['ok' => true];
        } catch (\Throwable $e) {
            return ['ok' => false, 'msg' => $e->getMessage()];
